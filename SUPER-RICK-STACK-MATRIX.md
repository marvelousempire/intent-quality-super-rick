...
**Last synced:** Saturday, July 11, 2026 at 4:00 PM HST · pulls from modality Ledgers + Why Guides + live receipts · ninth Standard Stack registered (`standard-communication-stack`) · current 20-run Parakeet Realtime EOU receipt synchronized without promoting failed latency gates

| Modality | Component | State now | Grade | Strongest-available (2026) | Action to A | Who |
|----------|-----------|-----------|-------|---------------------------|-------------|-----|
| **Voice · STT final** | Parakeet v3 via `mw` (M5 ANE cascade preferred) | ✅ live · **residency VERIFIED-PINNED** | **A** | Parakeet Realtime EOU 120M streaming is now measured `partial` (next row), not promoted | **resolved 2026-07-10:** source-verified — FluidAudio defaults `.cpuAndNeuralEngine` + int8-GPU coercion; live re-measure 99 ms cold / 45–53 ms warm, exact transcript (receipt `~/.nephew/run/ane-residency-receipt.json` → resolution) | voice |
| **Voice · STT streaming/EOU** | Parakeet Realtime EOU 120M via FluidAudio/Core ML (M5 resident) | ⚠️ `partial` · measured across **20** real-time runs · **both latency gates fail** | — | Local PCM16 streaming with partial, EOU, and exact-final behavior; public distribution remains unproven | First usable partial p95 **523 ms from measured speech onset / 643 ms from first PCM** vs ≤300 ms; semantic EOU p95 **457 ms** vs ≤450 ms (7 ms miss). Reduce both, expose the loaded source revision, then repeat public/physical distribution | voice + communication |
...
| **Communication · unified contract** | `standard-communication-stack` — the ninth Standard Stack; Voice + Text/Chat + Vision + Visual + Knowledge + Avatar in one person-scoped session | ✅ TypeScript/Swift contracts live · ⚠️ product runtime remains `partial` / `candidate` | — | One versioned cross-client contract with resumable events, turn identity, WebRTC, and secure Opus WebSocket fallback boundaries | Keep generators/fixtures synchronized; prove authenticated public SSE/realtime media, physical mobile behavior, and six-division continuity before promoting the product path | communication + Nephew + Presence |
...
| **RAG · coverage** | corpus indexed — **+ Super Rick standards shelf 2026-07-10** | 🟡 improving | **C+** | full repo + plans + apps indexed | **measured 2026-07-10:** `nephew-standards` live (432 pts / 133 files: the then-current 8 standard-* stacks + matrix + optimus, nephew 1.92.242-243); retrieve probe reranked hit **0.979** on avatar silicon law, 1.74 s E2E; in `DEFAULT_COLLECTIONS` + inventory + `standards` reindex lane. The ninth `standard-communication-stack` was published later and is not included in that receipt. Still open to A: communication/sibling app reindex + on-save reindex (Plan 0229) | RAG |
...
- Communication · [`standard-communication-stack/understandings/WHY-GUIDE.md`](https://git.jailynmarvin.com/marvelousempire/standard-communication-stack/src/branch/main/understandings/WHY-GUIDE.md)
